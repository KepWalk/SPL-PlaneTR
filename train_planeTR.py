...
class AddGaussianNoise(object):
    def __call__(self, img):
        ...
        img = np.array(img)
        noise = np.random.randn(*img.shape) * np.random.randint(0, 25 + 1)
...
def load_dataset(cfg, args):
    transforms = tf.Compose([
        tf.ColorJitter(brightness=[0.8, 1.2], contrast=[0.8, 1.2], saturation=[0.8, 1.2], hue=[-0.1, 0.1]),
        AddGaussianNoise(),
        tf.ToTensor(),
...
def train(cfg, logger):
    ...
    for p in network.module.transformer.encoder.layers.parameters():
        p.require_grad = False
    for p in network.module.backbone.parameters():
        p.require_grad = False
