...
def eval(cfg, logger):
    ...
    with torch.no_grad():
        for iter, sample in enumerate(data_loader):
            ...
            # ---------------------------------- debug: visualization
            if cfg.if_debug and args.local_rank == 1 and iter % cfg.debug_interval == 0:
                if_save_cloud = True
        ...
        if cfg.save_path != 'None':
            ...
            mine_recalls_pixel = {"SPL-PlaneTR": pixelDepth_recall_curve / len(data_loader) * 100}
            mine_recalls_plane = {"SPL-PlaneTR": planeDepth_recall_curve[:, 0] / planeDepth_recall_curve[:, 1] * 100}
            ...
            normal_recalls_pixel = {"SPL-planeTR": pixelNorm_recall_curve / len(data_loader) * 100}
            normal_recalls_plane = {"SPL-planeTR": planeNorm_recall_curve[:, 0] / planeNorm_recall_curve[:, 1] * 100}
