...
class scannetv1_PlaneDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt_plane_pixel_centers = np.zeros([2, 192, 256], dtype=np.float64)  # 2, 192, 256
        if self.predict_center:
            for i in range(num_planes):
                plane_mask = gt_segmentation == i
                pixel_num = plane_mask.sum()
                plane_mask = plane_mask.astype(np.float64)
                ...
                center_map = np.zeros([2, 192, 256], dtype=np.float64)  # 2, 192, 256
