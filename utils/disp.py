...
def plot_depth_recall_curve(method_recalls, type='', save_path=None, method_color=None):
    ...
    if type in ['pixel', 'PIXEL', 'Pixel']:
        ...
        recall_planeTR = np.array(
            [0., 30.55, 52.83, 63.96, 69.78, 73.71, 76.44, 77.82, 78.99, 79.62, 80.04, 80.41, 80.52])
        ...
        recall_planeTR = np.array(
            [0., 23.85, 40.73, 49.39, 53.85, 56.76, 58.51, 59.56, 60.46, 60.94, 61.27, 61.40, 61.49])
        pre_defined_recalls['PlaneTR'] = recall_planeTR
    ...
    markers = {'PlaneNet': 'o', 'PlaneAE': '*', 'PlaneTR': '.'}
    colors = {'PlaneNet': 'gray', 'PlaneAE': '#FFCC99', 'PlaneTR': 'mediumaquamarine'}
...
def plot_normal_recall_curve(method_recalls, type='', save_path=None, method_color=None):
    ...
    if type in ['pixel', 'PIXEL', 'Pixel']:
        ...
        recall_planeTR = np.array(
            [0., 30.08, 59.44, 70.55, 75.50, 77.33, 78.42, 79.03, 79.46, 79.70, 80.03, 80.19, 80.24])
        pre_defined_recalls['PlaneTR'] = recall_planeTR
    else:
        recall_planeAE = np.array(
        ...
        recall_planeTR = np.array(
            [0., 21.93, 43.14, 51.86, 55.54, 57.53, 58.62, 59.28, 59.78, 60.06, 60.44, 60.55, 60.68])
        pre_defined_recalls['PlaneTR'] = recall_planeTR
